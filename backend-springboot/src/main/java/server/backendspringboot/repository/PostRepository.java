...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT t2.username as name, t1.* FROM post t1 join account t2 ON t1.account_id = t2.account_id WHERE t1.tag_id = 48 AND t1.account_id NOT IN (SELECT s1.blocked FROM people_blocked s1 WHERE s1.account_id = 65);", nativeQuery = true)
    List<Post> NO_BLOCKED_getPostByTag_id(long tagId);
